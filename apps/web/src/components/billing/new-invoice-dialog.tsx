        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
                ...
                {doctors.map(d => (
                  <button key={d.id} type="button"
                    ...
                    <div className={cn(
                      ...
                      selectedDoctorId === d.id ? 'bg-primary text-white' : 'bg-muted text-muted-foreground'
                    )}>
                      {d.firstName[0]}{d.lastName[0]}
                    </div>
                    <div>
                      <p className={cn('text-sm font-medium', selectedDoctorId === d.id ? 'text-primary' : 'text-foreground')}>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <label className="text-sm font-semibold text-foreground">Conceptos</label>
            ...
            <div className="space-y-3">
              {lines.map((line, idx) => (
                <div key={idx} className="border border-border rounded-xl p-3 bg-muted/50">
                  {/* Row 1: service selector */}
                  <div className="flex gap-2 mb-2">
                    <div className="flex-1">
                      <label className="block text-xs text-muted-foreground mb-1">Servicio / Descripción</label>
                    ...
                    <div className="mb-2">
                      ...
                      {line.description && (
                        <label className="flex items-center gap-2 mt-1.5 cursor-pointer select-none">
                          ...
                          <span className="text-xs text-muted-foreground">Guardar en catálogo de servicios</span>
                        </label>
                      )}
                    </div>
                  )}
            ...
            {/* Total summary */}
            <div className="bg-primary/10 rounded-xl p-4 mt-3 space-y-1">
              ...
              <div className="flex justify-between text-base font-bold text-foreground border-t border-primary/30 pt-2 mt-1">
          ...
          {/* Payment method */}
          <div>
            <label className="block text-sm font-semibold text-foreground mb-2">Método de pago</label>
                ...
                return (
                  <button key={opt.value} type="button"
                    ...
                    className={cn(
                      ...
                      active ? 'border-primary bg-primary/10' : 'border-border bg-card hover:border-border'
                    )}>
                    <Icon className={cn('w-5 h-5', active ? 'text-primary' : 'text-muted-foreground')} />
                    <p className={cn('text-xs font-semibold leading-tight', active ? 'text-primary' : 'text-muted-foreground')}>
                      {opt.label}
                    </p>
                    {opt.desc && (
            ...
            {payMethod === 'INSURANCE' && (
              <div className="mt-2 space-y-2">
                <div>
                  <label className="block text-xs text-muted-foreground mb-1">Aseguradora *</label>
                ...
                <div>
                  <label className="block text-xs text-muted-foreground mb-1">Número de autorización (opcional)</label>
            ...
            {payMethod !== '' && payMethod !== 'INSURANCE' && (
              <div className="mt-2">
                <label className="block text-xs text-muted-foreground mb-1">Referencia (opcional)</label>
                <input
                  type="text"
                  value={payReference}
          ...
          {/* Notes */}
          <div>
            <label className="block text-sm font-medium text-foreground/80 mb-1">Notas internas</label>
          ...
          <div className="flex gap-3">
            <button type="button" onClick={onClose}
              className="flex-1 py-2.5 border border-border rounded-lg text-sm font-medium text-foreground/80 hover:bg-muted/50">
              Cancelar
            </button>
            <button type="submit" disabled={loading}
              className="flex-1 py-2.5 bg-primary hover:bg-primary disabled:opacity-50 text-white rounded-lg text-sm font-medium">

  ...
  return (
    <div ref={ref} className="relative">
      <button type="button" onClick={() => setOpen(o => !o)}
        className="w-full px-3 py-2 border border-border rounded-lg text-sm text-left focus:outline-none focus:ring-2 focus:ring-primary flex items-center justify-between bg-card">
        <span className={selected ? 'text-foreground' : 'text-muted-foreground'}>{displayValue}</span>
        <Clock className="w-4 h-4 text-muted-foreground" />
      </button>
      {open && (
        <div className="absolute z-50 mt-1 bg-card border border-border rounded-xl p-3 w-72">
          <div className="flex items-center justify-between mb-2">
            <button type="button" onClick={prevMonth} className="p-1 hover:bg-muted rounded-lg"><ChevronLeft className="w-4 h-4" /></button>
            <span className="text-sm font-semibold text-foreground">{MESES_ES[viewMonth]} {viewYear}</span>
            <button type="button" onClick={nextMonth} className="p-1 hover:bg-muted rounded-lg"><ChevronRight className="w-4 h-4" /></button>
          </div>
          <div className="grid grid-cols-7 mb-1">
            {DIAS_ES.map(d => <p key={d} className="text-center text-xs font-medium text-muted-foreground py-1">{d}</p>)}
          </div>
            ...
            {cells.map((day, i) => {
              ...
              return (
                <button key={i} type="button" disabled={isPast} onClick={() => select(day)}
                  className={`text-sm rounded-lg py-1 transition-colors ${isSelected ? 'bg-primary text-white font-semibold' : isPast ? 'text-muted-foreground/60 cursor-not-allowed' : 'hover:bg-primary/10 text-foreground'}`}>
                  {day}
                </button>
              )
            })}
          ...
          <div className="border-t border-border mt-2 pt-2 flex justify-end">
            <button type="button" onClick={() => { const t = new Date(); select(t.getDate()); setViewYear(t.getFullYear()); setViewMonth(t.getMonth()) }}
              className="text-xs text-primary hover:underline font-medium">Hoy</button>
          </div>
        </div>
      )}
    </div>
...
export function NewAppointmentDialog({ defaultDate, onClose, onCreated }: NewAppointmentDialogProps) {
  ...
  function slotClass(slot: string): string {
    ...
    if (sel)    return 'bg-primary text-white font-semibold border border-primary'
    if (booked) return 'bg-muted text-muted-foreground cursor-not-allowed line-through border border-border'
    if (past)   return 'bg-muted/50 text-muted-foreground/60 cursor-not-allowed border border-border'
    return 'bg-card text-foreground/80 hover:bg-primary/10 hover:border-primary/50 border border-border cursor-pointer'
  }
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-card rounded-2xl w-full max-w-lg max-h-[92vh] overflow-y-auto">
        {/* Header */}
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-5">
              ...
              <>
                ...
                {patients.length > 0 && !selectedPatientId && (
                  <div className="mt-1 border border-border rounded-lg overflow-hidden">
                    ...
                    <button type="button" onClick={switchToNew}
                      className="w-full px-3 py-2 text-left text-sm text-primary hover:bg-primary/10 flex items-center gap-2 border-t border-border">
                ...
                {selectedPatientId && <p className="text-xs text-success mt-1">✓ {selectedPatientName}</p>}
              </>
            ) : (
              <div className="space-y-2 bg-primary/10 border border-primary/30 rounded-lg p-3">
                <p className="text-xs text-primary font-medium">Registro rápido — completa el expediente después</p>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-xs text-muted-foreground mb-1">Nombre <span className="text-red-500">*</span></label>
                  ...
                  <div>
                    <label className="block text-xs text-muted-foreground mb-1">Apellido <span className="text-red-500">*</span></label>
                ...
                <div>
                  <label className="block text-xs text-muted-foreground mb-1">Teléfono WhatsApp <span className="text-red-500">*</span></label>
                  <div className="flex">
                    <span className="inline-flex items-center px-2 border border-r-0 border-border rounded-l-lg bg-muted/50 text-xs text-muted-foreground font-medium select-none">+52</span>
                    <input
                      type="tel"
                      inputMode="numeric"
                      ...
                      maxLength={10}
                      className="flex-1 px-3 py-2 border border-border rounded-r-lg text-sm bg-card focus:outline-none focus:ring-2 focus:ring-primary"
                    />
                  </div>
          ...
          {/* ── FECHA ── */}
          <div>
            <label className="block text-sm font-medium text-foreground/80 mb-1.5">Fecha</label>
          ...
          <div>
            ...
            : scheduleSlots.length === 0 ? (
              <p className="text-sm text-warning bg-warning/10 px-3 py-2 rounded-lg">
              ...
              <>
                <div className="flex gap-4 mb-2 text-xs text-muted-foreground">
                  <span className="flex items-center gap-1">
                    <span className="w-3 h-3 rounded bg-card border border-border inline-block" /> Disponible
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-3 h-3 rounded bg-muted border border-border inline-block" /> Ocupado
                  </span>
                  <span className="flex items-center gap-1">
                    <span className="w-3 h-3 rounded bg-muted/50 border border-border inline-block" /> Pasado
                  </span>
                </div>
                <div className="grid grid-cols-6 gap-1">
          ...
          {/* ── DURACIÓN ── */}
          <div>
            <label className="block text-sm font-medium text-foreground/80 mb-1.5">Duración</label>
            <div className="flex gap-2">
              {DURATIONS.map(d => (
                <button key={d.value} type="button" onClick={() => setDurationMins(d.value)}
                  className={`flex-1 py-2 rounded-lg text-xs font-medium border transition-colors ${
                    durationMins === d.value ? 'bg-primary text-white border-primary' : 'border-border text-muted-foreground hover:border-primary/50'
                  }`}>
                  {d.label}
                </button>
              ))}
          ...
          {/* ── MODALIDAD ── */}
          <div>
            <label className="block text-sm font-medium text-foreground/80 mb-1.5">Modalidad</label>
            <div className="flex gap-2">
              {(['IN_PERSON', 'TELEMEDICINE'] as const).map(m => (
                <button key={m} type="button" onClick={() => setMode(m)}
                  className={`flex-1 py-2 rounded-lg text-sm font-medium border transition-colors ${
                    mode === m ? 'bg-primary text-white border-primary' : 'border-border text-muted-foreground hover:border-primary/50'
                  }`}>
          ...
          {/* ── MOTIVO DE CONSULTA ── */}
          <div>
            <label className="block text-sm font-medium text-foreground/80 mb-1.5">Motivo de consulta</label>
            <div className="flex flex-wrap gap-1.5">
              ...
              <button type="button"
                ...
                className={`px-3 py-1 rounded-full text-xs font-medium border transition-colors ${
                  showCustom ? 'bg-foreground text-white border-foreground' : 'border-dashed border-border text-muted-foreground hover:border-foreground/80 bg-card'
                }`}>
            ...
            {!showCustom && complaint && <p className="text-xs text-primary mt-1">✓ {complaint}</p>}
          </div>
          ...
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onClose}
              className="flex-1 py-2.5 border border-border rounded-lg text-sm font-medium text-foreground/80 hover:bg-muted/50 transition-colors">
              Cancelar
            </button>
            <button type="submit" disabled={loading || !selectedTime || (isStaffRole && !doctorId)}
              className="flex-1 py-2.5 bg-primary hover:bg-primary/90 disabled:opacity-50 text-white rounded-lg text-sm font-medium transition-colors">
              {loading
